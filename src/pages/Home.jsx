...
export default function Home() {
  ...
  const requestPushPermission = () => {
    if ("Notification" in window) {
      Notification.requestPermission().then(permission => {
        if (permission === "granted") alert('تم تفعيل الإشعارات! ✅');
      });
    }
  };
  ...
  const getNotifStyle = (title = '', msg = '') => {
    ...
    if (text.includes('قبول') || text.includes('تسليم')) return { icon: <CheckCircle2 size={20}/>, color: 'text-green-500', bg: 'bg-green-500/10' };
    if (text.includes('رفض') || text.includes('فشل')) return { icon: <XCircle size={20}/>, color: 'text-red-500', bg: 'bg-red-500/10' };
    return { icon: <Zap size={20}/>, color: 'text-purple-500', bg: 'bg-purple-500/10' };
  };

  return (
    /* 🟢 التعديل الجوهري: h-screen بدلاً من min-h-screen لضبط الارتفاع */
    <div className="flex flex-col h-screen w-full bg-[#050505] text-white overflow-hidden relative" dir="rtl">

      {/* ======================= الهيدر العلوي (ثابت) ======================= */}
      <header className="px-6 py-5 flex justify-between items-center z-40 bg-[#050505]/90 backdrop-blur-2xl border-b border-white/5 shrink-0">
          ...
          <div className="w-10 h-10 rounded-xl p-0.5 shadow-lg shadow-orange-500/10" style={{ background: `linear-gradient(to top right, ${themeColor}, #dc2626)` }}>
            <div className="w-full h-full bg-[#050505] rounded-[10px] flex items-center justify-center overflow-hidden">
              {settings?.appLogo ? (
                ...
                <span className="font-black text-white text-xs">BADR</span>
              )}
            </div>
          </div>
          <div className="flex flex-col">
            <span className="text-white font-black text-sm">{settings?.appName || 'متجر بدر'}</span>
            <span className="text-gray-400 font-bold text-[10px]">{greeting}</span>
          </div>
        </div>

        <div className="flex gap-3 flex-1 justify-end">
          <div className={`flex items-center transition-all duration-500 ${isSearchOpen ? 'w-full sm:w-64 bg-white/5 border border-white/10 rounded-full px-3' : 'w-10 bg-transparent'}`}>
            <button onClick={() => setIsSearchOpen(true)} className={`h-10 flex items-center justify-center text-gray-400 ${!isSearchOpen && 'w-10 rounded-full bg-white/5'}`}>
              <Search size={18} />
            </button>
            {isSearchOpen && (
              <input type="text" placeholder="بحث..." autoFocus onBlur={() => setIsSearchOpen(false)} className="bg-transparent border-none outline-none text-xs text-white w-full px-2" />
            )}
          </div>

          <button onClick={handleOpenNotifs} className="w-10 h-10 shrink-0 rounded-full bg-white/5 flex items-center justify-center text-gray-400 relative border border-white/5">
      ...
      <div className="fixed bottom-6 left-6 right-6 z-40">
        <div className="bg-[#121217]/95 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-2 flex justify-between items-center shadow-2xl">
          {[
            ...
            return (
              <button key={tab.id} onClick={() => { setActiveTab(tab.id); setActiveCategory(null); }} className={`relative flex-1 flex flex-col items-center justify-center gap-1.5 py-3 rounded-full transition-all ${isActive ? 'text-white' : 'text-gray-500'}`}>
                {isActive && <div className="absolute inset-0 bg-white/10 rounded-[2rem] scale-100 shadow-inner"></div>}
      ...
      <div className={`fixed inset-0 z-[120] flex justify-end transition-all duration-500 ${showNotifs ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`} dir="rtl">
        ...
        <div className={`w-full max-w-sm bg-[#0a0a0c]/95 backdrop-blur-3xl h-full border-r border-white/10 shadow-2xl relative z-10 flex flex-col transition-transform duration-500 ${showNotifs ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="p-6 border-b border-white/5 flex justify-between items-center bg-black/20">
             ...
             <button onClick={() => setShowNotifs(false)} className="p-2 bg-white/5 rounded-xl text-gray-400"><X size={20}/></button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {displayNotifs.length > 0 ? (
              displayNotifs.map((n) => {
                ...
                return (
                  <div key={n.id} className="bg-[#121217] rounded-[1.5rem] border border-white/5 p-4 flex gap-4 items-start relative overflow-hidden group hover:bg-[#18181c] transition-all">
                    <div className={`absolute left-0 top-0 bottom-0 w-1 ${style.bg.replace('bg-', 'text-')} opacity-80`}></div>
                    <div className={`w-12 h-12 shrink-0 rounded-[1.2rem] flex items-center justify-center border ${style.bg} ${style.color} shadow-inner`}>{style.icon}</div>
                    <div className="flex-1">
                      <h4 className="text-sm font-black text-white pr-2">{n.title}</h4>
                      <p className="text-[11px] font-bold text-gray-400 my-1">{n.message || n.msg}</p>
              ...
              <div className="h-full flex flex-col items-center justify-center opacity-20"><Bell size={48} className="text-white" /></div>
            )}
          </div>
        </div>
      </div>
