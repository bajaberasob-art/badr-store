...
import { 
  ...
  Zap, Sparkles, CheckCircle2, XCircle, AlertTriangle 
} from 'lucide-react';
...
export default function OrderModal({ service, onClose }) {
  ...
  const handlePreConfirm = () => {
    ...
    if (!input.trim()) {
      return setCustomAlert({ type: 'error', title: 'بيانات ناقصة', msg: 'أدخل الآيدي أو رقم الحساب! 🆔' });
    }
  ...
  // 2. زر التنفيذ النهائي (الخصم الفعلي)
  const executeOrder = () => {
      ...
      <div className="relative bg-[#08080a] rounded-t-[3.5rem] border-t border-white/10 p-8 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] animate-in slide-in-from-bottom-full duration-700 max-h-[90vh] flex flex-col">
          ...
          <div className="absolute inset-0 z-40 bg-[#08080a]/95 backdrop-blur-xl rounded-t-[3.5rem] flex items-center justify-center p-6 animate-in zoom-in-95 duration-300">
            <div className="bg-[#121217] border border-orange-500/30 p-8 rounded-[3rem] shadow-[0_0_50px_rgba(249,115,22,0.15)] w-full max-w-sm">
               ...
               <div className="bg-black/50 p-5 rounded-[2rem] border border-white/5 space-y-4 mb-8">
                  ...
                  <div className="flex justify-between items-center border-b border-white/5 pb-3">
                     <span className="text-[10px] text-gray-500 font-black uppercase">الآيدي / الحساب</span>
                     <span className="text-sm text-orange-500 font-black tracking-widest">{confirmData.inputDetails}</span>
                  </div>
               ...
               <div className="flex gap-3">
                  <button onClick={executeOrder} className="flex-1 py-4 bg-orange-500 hover:bg-orange-600 rounded-[2rem] text-black font-black text-sm active:scale-95 transition-all shadow-lg">تأكيد وخصم</button>
                  <button onClick={() => setConfirmData(null)} className="flex-1 py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-[2rem] text-white font-black text-sm active:scale-95 transition-all">مراجعة وتعديل</button>
               </div>
            </div>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-8 pb-6">
           <div className="space-y-3">
              <label className="text-[10px] font-black text-gray-600 uppercase tracking-[0.3em] px-2 flex items-center gap-2"><Info size={12} /> البيانات المطلوبة (ID / الرقم)</label>
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="أدخل هنا..." className="w-full bg-[#121217] border border-white/5 p-6 rounded-[2rem] text-white font-black text-2xl text-center outline-none focus:border-orange-500/50 transition-all placeholder:text-gray-800" />
           </div>
        ...
        <div className="pt-6 shrink-0 border-t border-white/5 mt-2">
           {/* استدعاء دالة المراجعة والتأكيد بدل التنفيذ المباشر */}
