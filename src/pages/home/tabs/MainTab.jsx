...
export default function MainTab({ onNavigate }) {
  // 🟢 جلب الخدمات (services) للفلترة والأخبار
  ...
  return (
    <div className="p-5 pb-32 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      {/* كرت الرصيد الملكي (تمت إضافة العداد الذكي) */}
      <div className="relative rounded-[3rem] p-8 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] group active:scale-[0.98] transition-all duration-300"
        ...
        <div className="relative z-10 flex justify-between items-start">
           ...
           <button className="p-4 bg-white/10 backdrop-blur-xl rounded-[1.5rem] text-white border border-white/10 shadow-xl hover:bg-white/20 transition-colors">
      ...
      {/* الشريط الإخباري الحديث (أنيميشن خفيف بدون وسم Marquee القديم) */}
      <div className="flex items-center gap-3 bg-[#0a0a0c] p-3 rounded-2xl border border-white/5 shadow-inner overflow-hidden relative">
        ...
        <div className="flex-1 overflow-hidden relative h-5">
           {/* ستحتاج لإضافة كلاس animate-marquee في ملف tailwind.config.js أو index.css (لقد كتبته لك بطريقة تعمل بدون إعدادات مسبقة) */}
           <div className="whitespace-nowrap absolute right-0 text-[11px] font-black text-white/80 tracking-widest uppercase flex gap-8 animate-[marquee_15s_linear_infinite]">
      ...
      <div className="grid grid-cols-2 gap-4">
        {[
          ...
          <button key={cat.id} onClick={() => onNavigate(cat.id)}
                  className={`relative bg-gradient-to-br ${cat.color} p-7 rounded-[2.5rem] border border-white/5 flex flex-col items-center justify-center gap-4 hover:brightness-110 active:scale-95 transition-all overflow-hidden group shadow-[0_10px_30px_rgba(0,0,0,0.5)] ${cat.id === 'cards' ? 'col-span-2 flex-row py-5 h-24' : 'aspect-square'}`}>
             ...
             {hotDeals.map((service) => {
               ...
               return (
                 <div
                    ...
                    className="min-w-[80%] snap-center bg-[#121217] p-5 rounded-[2rem] border border-white/5 flex flex-col gap-4 group active:scale-[0.98] transition-all shadow-[0_15px_40px_rgba(0,0,0,0.5)] hover:border-white/10 cursor-pointer"
                 >
                    ...
                    <div>
                       ...
                       <div className="flex justify-between items-center">
                          ...
                          <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-orange-500 group-hover:text-black transition-colors duration-300">
                             <ChevronLeft size={16} />                                    
                          </div>
                       </div>
                    </div>
